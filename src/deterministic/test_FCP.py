...
def threshold_scores_with_nonempty_rows(score_matrix, cutoff):
    """
    Threshold segment-product scores into assortments, keeping one product if a
    segment would otherwise receive an empty bundle.
    """
    scores = np.asarray(score_matrix)
    pred_assort = (scores >= cutoff).astype(int)
    for k in range(pred_assort.shape[0]):
        if not np.any(pred_assort[k]):
            pred_assort[k, int(np.argmax(scores[k]))] = 1
    return pred_assort
...
def revenue_ratio(n, m, unit_cs, ship_cs, unit_us, Ns, opt_rev, pred_assort, time_limit=600):
    ...
    Z = model.addVars(m, predicted_bundles, vtype=GRB.CONTINUOUS,
    ...
    if model.Status == GRB.OPTIMAL:
        return model.ObjVal/opt_rev, float(model.Runtime)
...
def main(use_web_gcn=False, web_results_path=None):
    ...
    test_data_path = os.path.join(dir_path, "data", "deterministic", "test_m10n10_1e_3")
            ...
            try:
                ...
                if use_web_gcn:
                    ...
                    if isinstance(raw_out, dict):
                        ...
                        # If all products fall below cutoff for a segment, keep the top-scoring product.
                        pred_assort = threshold_scores_with_nonempty_rows(logits_nm.T, cutoff=0.0)
                    else:
                        # Legacy node-output model: keep original thresholding logic
                        output = raw_out[:n, :].detach().cpu().numpy()
                        prob_matrix = (np.exp(output) / (np.exp(output) + np.exp(1))).T
                        pred_assort = threshold_scores_with_nonempty_rows(prob_matrix, cutoff=0.5)
