...
def _save_seed_sample_results(rows: list, result_dir: str, test_folder_name: str):
    """Save the auditable FCPLS seed-by-sample long table."""
    if not rows:
        return None
    ...
    result_filename = f"test_result_FCPLS_{rows[0]['layers']}layer_{test_folder_name}_seed_sample.csv"
    result_path = os.path.join(result_dir, result_filename)
                    ...
                    for sd, mdl, path in models_by_layer[nl]:
                        try:
                            ...
                            seed_sample_rows_by_layer[nl].append({
                                ...
                                "m_segments": segment_num,
                                "n_products": n,
                                "competitive_ratio": float(best_rev),
                                "runtime_ratio": float(time_ratio),
                                "total_time": float(total_time),
                                "gcn_time": float(gcn_time),
                                "initial_milp_time": float(initial_milp_time),
                                "local_search_time": float(local_search_time),
                                "gnn_gurobi_time": float(gnn_gurobi_end_time),
                            })
